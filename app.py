...
from PyQt6.QtGui import QAction, QColor, QFont, QPainter, QPen, QPolygonF
...
class Schematic(QWidget):
    ...
    def _paint_legacy(self, event) -> None:
        painter = QPainter(self)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        painter.fillRect(self.rect(), QColor("#ffffff"))
        if not self.current:
        ...
        area = self.rect().adjusted(18, 34, -18, -62)
        ...
        rects = {
            "Compressor": box(0.02, 0.34, 0.16, 0.18),
            "Condenser": box(0.29, 0.34, 0.18, 0.18),
            "TXV": box(0.60, 0.34, 0.10, 0.18),
            "Evaporator": box(0.79, 0.34, 0.19, 0.18),
            "Suction header": box(0.79, 0.72, 0.19, 0.14),
        }
        active = QColor("#087f73")
        structure = QColor("#344054")
        conceptual = QColor("#b66a0a")

        compressor = rects["Compressor"]
        condenser = rects["Condenser"]
        txv = rects["TXV"]
        evaporator = rects["Evaporator"]
        header = rects["Suction header"]
        component_ids = {
        ...
        defrost_active = self.valve_states["hot_gas_solenoid"]
        normal_color = QColor("#98a2b3") if defrost_active else active
        suction_stop_point = QPointF(
            evaporator.center().x(),
            (evaporator.bottom() + header.top()) / 2,
        )
        normal_segments = [
            (QPointF(compressor.right(), compressor.center().y()), QPointF(condenser.left(), condenser.center().y())),
            (QPointF(condenser.right(), condenser.center().y()), QPointF(txv.left(), txv.center().y())),
            (QPointF(txv.right(), txv.center().y()), QPointF(evaporator.left(), evaporator.center().y())),
            (QPointF(evaporator.center().x(), evaporator.bottom()), QPointF(suction_stop_point.x(), suction_stop_point.y() - 12)),
            (QPointF(suction_stop_point.x(), suction_stop_point.y() + 12), QPointF(header.center().x(), header.top())),
        ...
        for start, end in normal_segments:
            self._draw_arrow(painter, start, end, normal_color)

        for label, rect in rects.items():
            painter.setBrush(QColor("#f5f8fb"))
            component = next((key for key, value in component_ids.items() if value == rect), "")
            selected = component == self.selected_component
        ...
        self._label(
            painter,
            compressor.left(),
            compressor.bottom() + 20,
            f"Speed {self.current.inputs.compressor_speed_frac * 100:.0f}%  •  Power {_format_quantity('W_comp_w', q['W_comp_w'])}",
            active,
        )
        self._label(
            painter,
            condenser.left(),
            condenser.bottom() + 20,
            f"Air {self.current.inputs.condenser_airflow_frac * 100:.0f}%  •  Discharge {_format_quantity('p_discharge_pa', q['p_discharge_pa'])}",
            active,
        )
        self._label(
            painter,
            txv.left(),
            txv.bottom() + 20,
            f"Open {q['txv_opening_frac'].value * 100:.0f}%  •  SH {_format_quantity('superheat_mixed_k', q['superheat_mixed_k'])}",
            active,
        )
        self._label(
            painter,
            evaporator.left(),
            evaporator.bottom() + 20,
            f"Air {self.current.inputs.airflow_frac * 100:.0f}%  •  Capacity {_format_quantity('Q_evap_w', q['Q_evap_w'])}",
            active,
        )
        self._label(
            painter,
            header.left() - 42,
            header.bottom() + 20,
            f"Suction {_format_quantity('p_suction_pa', q['p_suction_pa'])}  •  Flow {_format_quantity('m_dot_kg_s', q['m_dot_kg_s'])}",
            active,
        )

        if self.show_conceptual:
            tee = QPointF(area.left() + 0.235 * w, compressor.center().y())
            hot_gas_valve = box(0.43, 0.02, 0.19, 0.16)
            coil_outlet_tee = QPointF(
                evaporator.center().x(),
                evaporator.bottom() + 0.035 * h,
            )
            hot_gas_drop_x = area.left() + 0.992 * w
            hot_gas_route = [
                (tee, QPointF(tee.x(), hot_gas_valve.center().y())),
                (QPointF(tee.x(), hot_gas_valve.center().y()), QPointF(hot_gas_valve.left(), hot_gas_valve.center().y())),
                (QPointF(hot_gas_valve.right(), hot_gas_valve.center().y()), QPointF(hot_gas_drop_x, hot_gas_valve.center().y())),
                (QPointF(hot_gas_drop_x, hot_gas_valve.center().y()), QPointF(hot_gas_drop_x, coil_outlet_tee.y())),
                (QPointF(hot_gas_drop_x, coil_outlet_tee.y()), coil_outlet_tee),
            ]
            hot_gas_open = self.valve_states["hot_gas_solenoid"]
            hot_gas_pen = QPen(conceptual, 3 if hot_gas_open else 2.5)
            if not hot_gas_open:
                hot_gas_pen.setStyle(Qt.PenStyle.DashLine)
            painter.setPen(hot_gas_pen)
            for start, end in hot_gas_route:
                if hot_gas_open:
                    self._draw_arrow(painter, start, end, conceptual)
                else:
                    painter.drawLine(start, end)
            painter.setBrush(QColor("#fff7e8"))
            painter.setPen(QPen(conceptual, 1.5, Qt.PenStyle.DashLine))
            painter.drawRoundedRect(hot_gas_valve, 6, 6)
            painter.setPen(QColor("#7a4300"))
            hot_gas_state = "OPEN" if hot_gas_open else "CLOSED"
            painter.drawText(hot_gas_valve, Qt.AlignmentFlag.AlignCenter, f"Hot-gas defrost solenoid\n{hot_gas_state}")

            suction_open = self.valve_states["liquid_line_solenoid"]
            painter.setBrush(QColor("#e9f8f4") if suction_open else QColor("#f2f4f7"))
            painter.setPen(QPen(active if suction_open else QColor("#667085"), 2.5))
            painter.drawEllipse(suction_stop_point, 11, 11)
            painter.drawText(
                QRectF(suction_stop_point.x() - 170, suction_stop_point.y() + 14, 150, 34),
                Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter,
                f"Liquid-line solenoid\n{'OPEN' if suction_open else 'CLOSED'}",
            )

            liquid_tee = QPointF(condenser.right() + 0.055 * w, condenser.center().y())
            evaporator_inlet = QPointF(evaporator.left(), evaporator.center().y())
            bypass_y = txv.top() - 0.10 * h
            check_point = QPointF((liquid_tee.x() + evaporator_inlet.x()) / 2, bypass_y)
            bypass_route = [
                (evaporator_inlet, QPointF(evaporator_inlet.x(), bypass_y)),
                (QPointF(evaporator_inlet.x(), bypass_y), QPointF(liquid_tee.x(), bypass_y)),
                (QPointF(liquid_tee.x(), bypass_y), liquid_tee),
            ]
            painter.setPen(QPen(conceptual, 3 if hot_gas_open else 2.5, Qt.PenStyle.SolidLine if hot_gas_open else Qt.PenStyle.DashLine))
            for start, end in bypass_route:
                if hot_gas_open:
                    self._draw_arrow(painter, start, end, conceptual)
                else:
                    painter.drawLine(start, end)
            self._draw_check_valve(painter, check_point, conceptual, reverse=True)
            painter.setPen(QColor("#7a4300"))
            painter.drawText(
                QRectF(check_point.x() - 95, check_point.y() - 34, 190, 24),
            ...
            if hot_gas_open:
                reverse_coil_route = [
                    (coil_outlet_tee, QPointF(evaporator.center().x(), evaporator.bottom())),
                    (QPointF(evaporator.center().x(), evaporator.bottom()), evaporator.center()),
                    (evaporator.center(), evaporator_inlet),
                ]
                painter.setPen(QPen(conceptual, 3))
                for start, end in reverse_coil_route:
                    self._draw_arrow(painter, start, end, conceptual)

            painter.drawText(
                QRectF(area.left(), area.top() - 30, area.width(), 24),
                Qt.AlignmentFlag.AlignCenter,
                "GENERIC REVERSE-FLOW HOT-GAS DEFROST — PATH ONLY, NOT RLN5MA-VERIFIED",
            )
            component_ids["hot_gas_solenoid"] = hot_gas_valve
            component_ids["liquid_line_solenoid"] = QRectF(
                suction_stop_point.x() - 18,
                suction_stop_point.y() - 18,
                36,
                36,
            )
            component_ids["check_valve"] = QRectF(
                check_point.x() - 20,
                check_point.y() - 20,
                40,
                40,
        ...
        painter.setPen(QPen(active, 3))
        painter.drawLine(20, legend_y, 50, legend_y)
        painter.setPen(QColor("#344054"))
        painter.drawText(58, legend_y + 5, "confirmed normal path / active flow")
        painter.setPen(QPen(conceptual, 2.5, Qt.PenStyle.DashLine))
        painter.drawLine(290, legend_y, 320, legend_y)
        painter.setPen(QColor("#344054"))
        painter.drawText(328, legend_y + 5, "conceptual or unverified")
        painter.drawText(540, legend_y + 5, "? exact component or location unknown")
        self._component_rects = component_ids
        painter.end()

    def _paint_field_v1(self, event) -> None:
        painter = QPainter(self)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        painter.fillRect(self.rect(), QColor("#ffffff"))
        if not self.current:
        ...
        area = self.rect().adjusted(18, 38, -18, -60)
        w, h = area.width(), area.height()
        ...
        components = {
            "compressor": ("Compressor", box(0.02, 0.38, 0.14, 0.17)),
            "high_pressure_switch": ("High-pressure\nswitch", box(0.055, 0.03, 0.15, 0.11)),
            "condensate_pan_coil": ("Condensate-pan\nhot-gas coil", box(0.28, 0.03, 0.17, 0.14)),
            "condenser": ("Condenser", box(0.49, 0.03, 0.16, 0.14)),
            "filter_drier": ("Filter-drier", box(0.69, 0.045, 0.10, 0.11)),
            "liquid_line_solenoid": ("Liquid-line solenoid", box(0.81, 0.035, 0.17, 0.13)),
            "txv": ("TXV", box(0.85, 0.25, 0.11, 0.13)),
            "distributor": ("Distributor", box(0.83, 0.48, 0.14, 0.15)),
            "evaporator_1": ("Evaporator circuit 1", box(0.58, 0.43, 0.18, 0.13)),
            "evaporator_2": ("Evaporator circuit 2", box(0.58, 0.67, 0.18, 0.13)),
            "header": ("Suction header", box(0.33, 0.69, 0.19, 0.14)),
            "hot_gas_solenoid": ("Hot-gas solenoid", box(0.30, 0.31, 0.19, 0.13)),
        }
        check_point = point(0.57, 0.375)
        components["check_valve"] = ("", QRectF(check_point.x() - 18, check_point.y() - 18, 36, 36))
        ...
        circuit_1 = components["evaporator_1"][1]
        circuit_2 = components["evaporator_2"][1]
        ...
        tee_1 = point(0.195, 0.465)
        tee_2 = point(0.245, 0.465)
        ...
        main_segments = [
            ...
            (QPointF(liquid_solenoid.right(), liquid_solenoid.center().y()), QPointF(txv.center().x(), liquid_solenoid.center().y())),
            (QPointF(txv.center().x(), liquid_solenoid.center().y()), QPointF(txv.center().x(), txv.top())),
            (QPointF(txv.center().x(), txv.bottom()), QPointF(distributor.center().x(), distributor.top())),
        ]
        ...
        hot_route = [
            (tee_2, QPointF(hot_solenoid.left(), hot_solenoid.center().y())),
            (QPointF(hot_solenoid.right(), hot_solenoid.center().y()), check_point),
            (check_point, point(0.99, 0.375)),
            (point(0.99, 0.375), QPointF(area.right() - 2, distributor.center().y())),
            (QPointF(area.right() - 2, distributor.center().y()), QPointF(distributor.right(), distributor.center().y())),
        ]
        ...
        painter.setPen(QColor("#7a4300"))
        painter.drawText(
            QRectF(check_point.x() - 48, check_point.y() - 31, 96, 22),
            Qt.AlignmentFlag.AlignCenter,
            "Check valve",
        )

        branch_point = point(0.80, 0.555)
        distributor_segments = [
            (QPointF(distributor.left(), distributor.center().y()), branch_point),
            (branch_point, QPointF(branch_point.x(), circuit_1.center().y())),
            (QPointF(branch_point.x(), circuit_1.center().y()), QPointF(circuit_1.right(), circuit_1.center().y())),
            (branch_point, QPointF(branch_point.x(), circuit_2.center().y())),
            (QPointF(branch_point.x(), circuit_2.center().y()), QPointF(circuit_2.right(), circuit_2.center().y())),
        ]
        painter.setPen(QPen(common_color, 3))
        for start, end in distributor_segments:
            self._draw_arrow(painter, start, end, common_color)

        merge = point(0.55, 0.76)
        return_segments = [
            (QPointF(circuit_1.left(), circuit_1.center().y()), QPointF(merge.x(), circuit_1.center().y())),
            (QPointF(merge.x(), circuit_1.center().y()), merge),
            (QPointF(circuit_2.left(), circuit_2.center().y()), QPointF(merge.x(), circuit_2.center().y())),
            (QPointF(merge.x(), circuit_2.center().y()), merge),
            (merge, QPointF(header.right(), header.center().y())),
            (QPointF(header.left(), header.center().y()), point(0.19, 0.76)),
            (point(0.19, 0.76), QPointF(compressor.center().x(), point(0.19, 0.90).y())),
            (QPointF(compressor.center().x(), point(0.19, 0.90).y()), QPointF(compressor.center().x(), compressor.bottom())),
        ]
        painter.setPen(QPen(common_color, 3))
        for start, end in return_segments:
            self._draw_arrow(painter, start, end, common_color)

        def selected(component: str) -> bool:
            if component in {"evaporator_1", "evaporator_2"}:
                return self.selected_component == "evaporator"
            return self.selected_component == component

        for component, (label, rect) in components.items():
            if component == "check_valve":
                continue
            valve = component in {"hot_gas_solenoid", "liquid_line_solenoid"}
            if valve:
                ...
                display = f"{label}\n{state}"
            elif component == "evaporator_1":
                display = f"Evaporator circuit 1\nSH {_format_quantity('superheat_circuit_k_1', self.current.quantities['superheat_circuit_k_1'])}"
                fill = QColor("#f5f8fb")
            elif component == "evaporator_2":
                display = f"Evaporator circuit 2\nSH {_format_quantity('superheat_circuit_k_2', self.current.quantities['superheat_circuit_k_2'])}"
                fill = QColor("#f5f8fb")
            else:
                display = label
                fill = QColor("#f5f8fb")
            painter.setBrush(fill)
            painter.setPen(QPen(QColor("#2563a6") if selected(component) else structure, 3 if selected(component) else 1.5))
            painter.drawRoundedRect(rect, 7, 7)
            painter.setPen(QColor("#172033"))
        ...
        painter.drawLine(20, legend_y, 50, legend_y)
        painter.setPen(QColor("#344054"))
        painter.drawText(58, legend_y + 5, "cooling path")
        painter.setPen(QPen(amber, 3))
        painter.drawLine(160, legend_y, 190, legend_y)
        painter.setPen(QColor("#344054"))
        painter.drawText(198, legend_y + 5, "hot-gas path")
        painter.setPen(QPen(structure, 2))
        painter.drawLine(315, legend_y, 345, legend_y)
        painter.drawText(353, legend_y + 5, "pressure-sensing branch")
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        area = self.rect().adjusted(18, 38, -18, -62)
        w, h = area.width(), area.height()
...
class ComponentControls(QFrame):
    ...
    CONFIG = {
        "compressor": (
            "Compressor",
            (("Compressor speed", "compressor_speed_frac", 50, 120),),
            "Change the compressor speed; suction and discharge pressures remain calculated outputs.",
        ),
        "condenser": (
            "Condenser",
            (
                ("Condenser airflow", "condenser_airflow_frac", 40, 120),
                ("Installed condenser size", "condenser_capacity_frac", 70, 130),
            ),
            "Fan airflow and installed condenser capacity are separate physical changes.",
        ),
        "txv": (
            "Thermostatic expansion valve",
            (
                ("Manual opening", "txv_opening_frac", 20, 100),
                ("Installed valve size", "txv_size_frac", 70, 130),
            ),
            "Opening and installed valve size are separate physical changes.",
        ),
        "evaporator": (
            "Evaporator",
            (
                ("Evaporator airflow", "airflow_frac", 40, 120),
                ("Installed evaporator size", "evaporator_capacity_frac", 70, 130),
            ),
            "Fan airflow and installed evaporator capacity are separate physical changes.",
        ),
        "header": (
            "Suction header",
            (),
    ...
    def __init__(self) -> None:
        ...
        self.rows: list[tuple[QWidget, QLabel, QSlider, QLabel]] = []
        for index in range(2):
            ...
            slider.valueChanged.connect(lambda value, row_index=index: self._slider_changed(row_index, value))
            value_label = QLabel()
            ...
            self.rows.append((row_widget, label, slider, value_label))
    ...
    def select_component(self, component: str, values: dict[str, float] | None = None) -> None:
        ...
        for index, row_items in enumerate(self.rows):
            row_widget, label, slider, value_label = row_items
            if index >= len(controls):
                row_widget.hide()
                continue
            row_widget.show()
            control_label, parameter, minimum, maximum = controls[index]
            label.setText(control_label)
            slider.setProperty("parameter", parameter)
            slider.blockSignals(True)
            slider.setRange(minimum, maximum)
            slider.setValue(round(self.values.get(parameter, 1.0) * 100))
    ...
    def _slider_changed(self, row_index: int, value: int) -> None:
        _, _, slider, value_label = self.rows[row_index]
        parameter = slider.property("parameter")
        if not parameter:
            return
        normalized = value / 100.0
        self.values[str(parameter)] = normalized
        value_label.setText(f"{value}%")
        self.inputChanged.emit(str(parameter), normalized)
