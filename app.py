...
class Schematic(QWidget):
    ...
    def _paint_field_v1(self, event) -> None:
        ...
        # Legend at the TOP, on the title line. It used to sit at the bottom and
        # consume the band the editor now occupies, which pushed the controls off
        # screen and forced scrolling.
        legend_y = int(area.top() - 24)
        legend_x = int(area.left())
        painter.setPen(QPen(teal, 3))
        painter.drawLine(legend_x, legend_y, legend_x + 24, legend_y)
        painter.setPen(QColor("#344054"))
        painter.drawText(legend_x + 30, legend_y + 5, "cooling path")
        painter.setPen(QPen(amber, 3))
        painter.drawLine(legend_x + 128, legend_y, legend_x + 152, legend_y)
        painter.setPen(QColor("#344054"))
        painter.drawText(legend_x + 158, legend_y + 5, "hot-gas path")
        painter.setPen(QPen(structure, 2))
        painter.drawLine(legend_x + 262, legend_y, legend_x + 286, legend_y)
        painter.drawText(legend_x + 292, legend_y + 5, "pressure-sensing branch")
