...
**Last updated: 2026-08-03. Gate: 0/6 — model does not integrate.**
...
**Compiler defects (M1a-M1e build/init) are resolved.** The model now builds and
initialises. It then hits a **singular linear system at t≈0.0553 s** — structural,
identical under CVode and DASSL. This is a genuine physics/formulation problem, not a
toolchain problem. See `NEXT_STEPS.md` Steps A/B.

Two real bugs were found and fixed getting here:
- `CoilAirSide.mo` had a duplicate-definition binding equation (over-determined by 2)
- Air/refrigerant heat exchange used mismatched units (W supplied where W/m² was required)
  — energy did not conserve across the interface

**Gate: 0/6** (`tests/test_scenarios.py`). The banner stays up. Nothing was weakened.
