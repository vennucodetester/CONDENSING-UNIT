...
8. **Gate Harness Enforcement & MD5 Verification:**
   Never run `pytest` directly after editing `.mo` files. Always run `./gate.sh` (or `bash ./gate.sh`) to rebuild, prove the FMU MD5 hash has changed, install the FMU, and execute tests. Treat any test results without a preceding `built ok: md5 ...` line as unreported.

9. **Modelica Variable Binding Rule:**
   In Modelica, `Real x = expr;` on a non-parameter variable is a BINDING EQUATION, not a start value. Passing parameter modifications on `Real` variables in component instantiations (e.g. `epsilon_v = 0.85` or `T_air_off_k = 278.15`) silently over-determines the model (+1 equation per instance). Use `(start = ...)` attributes for initial values or explicit runtime equations in the `equation` section.
